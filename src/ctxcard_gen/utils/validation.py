...
def validate_role_tags(modules: dict) -> Tuple[bool, List[str]]:
    ...
    for tag in all_role_tags:
        if not re.match(r"^[a-z_]+$", tag):
            errors.append(f"Invalid role tag format: '{tag}' (should be lowercase with underscores)")
...
def validate_naming_conventions(modules: dict, naming_patterns: Dict[str, str]) -> Tuple[bool, List[str]]:
    """
        ...
        for symbol in module_info.symbols:
            if symbol.kind in naming_patterns:
                ...
                if not re.match(pattern, symbol.name):
                    errors.append(f"Symbol '{symbol.name}' does not match naming pattern for {symbol.kind}: {pattern}")
