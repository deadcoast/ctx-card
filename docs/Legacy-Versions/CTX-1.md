...
Nice—here’s a single-file, no-deps **CTX-CARD generator** you can drop into any repo. It scans modules, extracts symbols (classes & functions for Python), builds compact indices, adds module-level edges from imports, and writes `CTXCARD.md` (or a delta vs an existing file).

It’s **language-aware for Python today** (AST-based), and **language-agnostic fallback** (just modules) for everything else. I kept it SRP-ish and readable so you can extend for TS/Go later.
...
- **ID/AL/NM**: Inserts sensible defaults so agents have immediate constraints.
- **MO**: Every code file becomes a module with stable `#id`.
- **SY**:

  - `#m.#0 | mod | <module_stem>` = module pseudo-symbol (anchor for edges).
  - Python: top-level `class` → `cls`, `def/async def` → `fn` with signatures if type hints are present.

- **ED**:

  - Module-level import edges from `#m.#0` → `#m'.#0`.
  - Relative imports are normalized best-effort; dotted names are resolved by matching last component to file stems in-repo (heuristic but effective).

- **CN/RV**: Minimal policy lines so the agent has some guardrails even if you don’t add more.
...
# Specify project slug and exclude tests
python ctxcard_gen.py --proj milkBottle --exclude "**/tests/**"

# Only Python files
python ctxcard_gen.py --include "**/*.py"

# Compute a Δ against an existing CTXCARD.md
python ctxcard_gen.py --delta-from CTXCARD.md --out CTXCARD.new.md
```

---

## Extend it (optional pointers)

- **TypeScript/JS**: add a lightweight parser (e.g., regex for `class`/`function`/`export`) or plug in `tree-sitter` if you’re okay with a dep. Map to `cls`/`fn` and fill `SY/SG`.
- **Function call graph** (Python): walk AST to collect `ast.Call` and resolve local function names → add `ED: #m.#sid -> #m'.#sid' | calls`.
- **DTO detection**: detect `@dataclass`, `pydantic.BaseModel` to emit `DT:` lines.
- **Error taxonomy**: collect custom Exceptions in code to populate `ER:`.

If you want, point this at one of your repos and I’ll tweak it to surface your specific DTOs/use-cases and seed better `AL/NM` defaults.
